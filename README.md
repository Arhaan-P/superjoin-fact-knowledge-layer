...
- `time_period` records what a forecast is _for_, not _when it was made_: an older vs. newer forecast for the same period can misread as a contradiction rather than a timing artifact (seen between an Economic Survey citation and an IMF report). Next step: a `forecast_vintage` field.
